/* Motor
  ...
  There are two modes for Servo use. Mode 0 uses a direct mapping of the right stick to the pan & tilt servos. Mode 1 uses a servo easing (smoothing) method, interpreting right stick movement as a change in position. To change modes, the Arduino expects a pan & tilt position of 255. As a result, the serial package should be 0x'#' XX XX FF FF
*/
#include <Servo.h>
...
int mode = 0; //0=direct servo mapping; 1=relative easing servos 
...
void setup()
{
  ...
  panEaser.begin(panServo, servoFrameMillis);
  tiltEaser.begin(tiltServo, servoFrameMillis);
...
{  
  ...
  if (Serial.available() > messageLength) {
    ...
    if(checkByte == '#') 
    {
      ...
      if (0xFF == panPos && 0xFF ==tiltPos){ //change modes
        mode = !mode;
      //turn the servos to position
      }else if (0 == mode) { //direct
        panServo.write(panPos);
        tiltServo.write(tiltPos);
      } else if (1 == mode) { //relative
        int deltaPan = int(panPos) - 90; //give us a number -90 to 90
        int deltaPanMagnitude = abs(deltaPan);

        int panSpeed= map(deltaPanMagnitude,0,90, 5000, 500);
        int newPan = prevPan + deltaPan;
        panEaser.easeTo(newPan,panSpeed);

        int deltaTilt = int(panTilt) - 90; //give us a number -90 to 90
        int deltaTiltMagnitude = abs(deltaTilt);

        int tiltSpeed= map(deltaTiltMagnitude,0,90, 5000, 500);
        int newTilt = prevTilt + deltaTilt;
        tiltEaser.easeTo(newTilt,tiltSpeed);

        prevPan = newPan;
        prevTilt= newTilt;
      }
      Serial.print('!'); //received successfully
    } //end if proper header
    else {
      //there's the possiblity that the '#' wasn't the first character sent
      //but loop() will loop, and we'll pull the next character in the buffer in.
    }
  } //end if (available > message length)
  } //end if (available())
